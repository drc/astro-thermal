...
interface ReceiptEncoder {
  ...
  image(img: Image, width: number, height: number, dither?: string): ReceiptEncoder;
...
const EncoderClass = ReceiptPrinterEncoder as unknown as ReceiptPrinterConstructor;
...
export async function printImage(image_url: string, alt_url?: string): Promise<void> {
  ...
  if (image.width === 0 || image.height === 0) throw new Error("Invalid image dimensions");
